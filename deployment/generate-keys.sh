...
CN = webhook-server.kube-system.svc
[ v3_req ]
...
[alt_names]
DNS.1 = webhook-server.kube-system.svc
EOF
...
openssl req -new -key webhook-server-tls.key -subj "/CN=webhook-server.kube-system.svc" -config server.conf \
